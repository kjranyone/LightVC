"""
Retrieval v2.1 — context size拡大 + F-ratio重み付き距離
"""
import sys, json
...
def build_context_key(mc, f0, spk_mean, ctx=0, use_delta=False, use_f0=False,
                      weights=None):
    T = len(mc)
    mc_norm = mc - spk_mean
    if weights is not None:
        mc_norm = mc_norm * weights[None, :]

    parts = []
    if ctx > 0:
        padded = np.pad(mc_norm, ((ctx, ctx), (0, 0)), mode="edge")
        ctx_frames = np.stack([padded[i:i+T] for i in range(2*ctx+1)], axis=-1)
        parts.append(ctx_frames.reshape(T, -1))
    else:
    ...
    if use_f0:
        f0_feat = np.zeros((T, 1), dtype=np.float32)
        voiced = f0 > 0
        if voiced.any():
            f0_feat[voiced, 0] = np.log(f0[voiced]).mean()
        parts.append(f0_feat)
...
def main():
    ...
    print("=== Retrieval v2.1 — Context + Weighting ===\n")
    ...
    fratio_path = Path("data/mc_fratio_weights.npy")
    fratio = np.load(fratio_path) if fratio_path.exists() else None
    if fratio is not None:
        inv_fratio = 1.0 / (fratio + 1e-6)
        inv_fratio = inv_fratio / inv_fratio.mean()
        print(f"F-ratio weights loaded: shape={fratio.shape}")
        print(f"  inv_fratio: min={inv_fratio.min():.3f} max={inv_fratio.max():.3f}")

    pairs = find_pairs(N_PAIRS)
    print(f"Pairs: {len(pairs)}\n")

    configs = [
        ("baseline",        0, False, False, None),
        ("ctx4",            4, False, False, None),
        ("ctx8",            8, False, False, None),
        ("ctx16",          16, False, False, None),
        ("ctx4_delta_f0",   4, True,  True,  None),
        ("ctx8_delta_f0",   8, True,  True,  None),
        ("ctx4_invfratio",  4, False, False, "inv_fratio"),
        ("ctx8_invfratio",  8, False, False, "inv_fratio"),
        ("ctx4_delta_f0_invfr", 4, True, True, "inv_fratio"),
        ("ctx8_delta_f0_invfr", 8, True, True, "inv_fratio"),
    ]
    ...
    for idx, p in enumerate(pairs):
        ...
        with torch.no_grad():
            ...
            for name, ctx, delta, f0reg, wtype in configs:
                w = None
                if wtype == "inv_fratio" and fratio is not None:
                    w = inv_fratio.astype(np.float32)

                src_keys = build_context_key(mc_s, f0_s, src_mean, ctx, delta, f0reg, w)
                bank_keys = build_context_key(bank_mc, bank_f0, tgt_mean, ctx, delta, f0reg, w)
                mc_pred = retrieve(src_keys, bank_keys, bank_mc, k=5, temp=1.0)
    ...
    print(f"\n{'='*60}")
    print(f"{'config':<25} {'mean':>8} {'std':>8} {'vs base':>8}")
    print(f"{'-'*55}")
    base_score = np.mean(results["baseline"])
    for name, *_ in configs:
        arr = np.array(results[name])
        diff = arr.mean() - base_score
        print(f"{name:<25} {arr.mean():>8.4f} {arr.std():>8.4f} {diff:>+8.4f}")
    ...
    if best_score >= 0.42:
        ...
        print("→ 0.42にほぼ到達、微調整で届く範囲")
    else:
        print("→ まだ不十分")

    out = {name: {"mean": float(np.mean(v)), "std": float(np.std(v))}
           for name, v in results.items()}
    with open("results/retrieval_v2_1.json", "w") as f:
