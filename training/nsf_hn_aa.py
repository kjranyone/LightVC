...
from nsf_hn import SourceModuleHnNSF, get_padding, init_weights, LRELU
from aa import AALeaky


class ResBlockAA(nn.Module):
    ...
    def __init__(self, cond_dim: int, sr: int = 44100, hop: int = 512,
        ...
        for i, (u, k) in enumerate(zip(upsample_rates, upsample_kernels)):
            ...
            for rk, rd in zip(resblock_ks, resblock_ds):
                self.resblocks.append(ResBlockAA(ch_out, rk, rd))
...
if __name__ == "__main__":
    ...
    g = NsfHifiGanAA(cond_dim=770, timbre_dim=192)
    missing, unexpected = g.load_state_dict(g0.state_dict(), strict=False)
    miss_params = [k for k in missing if "filt" not in k]
    print("warm-start: non-buffer missing keys:", miss_params)
    print("unexpected keys:", list(unexpected))
    cond = torch.randn(1, 770, 40); f0 = torch.rand(1, 40) * 200 + 120
    s = torch.randn(1, 192)
    y = g(cond, f0, s)
    print("params", round(sum(p.numel() for p in g.parameters()) / 1e6, 2), "M",
          "| out", tuple(y.shape), "expected T", 40 * 512)
