...
マイク入力をリアルタイムに変換してスピーカーに出力します。Converter
重みが未ロードでも起動できます — その場合は bypass モード（入力をそのまま
出力）で動作し、オーディオデバイスのテストに利用できます。

1. **モデルロード（任意）** — Converter重みとConfigを指定して「Load Converter」をクリック。未ロード時はStartボタンが「▶ Start (bypass)」になり、音声は変換されずそのまま出力される
2. **Start / Stop** — 「▶ Start」でリアルタイム変換を開始、「■ Stop」で停止
3. **ステータス** — 現在の状態を表示: `● LIVE`（変換中）/ `BYPASS`（バイパス中）/ `STOPPED`（停止中）
4. **モード切替** — Strict（0ms）/ Balanced（~40ms）/ Quality（~80ms）をノブで切替（Converterロード時のみ表示）
5. **Bypass** — 変換をバイパス（入力をそのまま出力）。Converter未ロード時は固定でON
6. **レベルメーター** — 入出力の音量を可視化
7. **メトリクス** — エンドツーエンドレイテンシ（ms）、RTF（リアルタイムファクター）、xrun（オーバーラン/アンダーラン）カウントを表示
8. **Audio Devices** — 認識されている入出力オーディオデバイスの一覧。ラジオボタンで入出力デバイスを個別に選択可能（未選択時はシステムデフォルト）
