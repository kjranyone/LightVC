...
> **2026-06-21更新**: 旧「Progressive RVQ-depth factorized FM heads」は、naive depth semantics と continuous latent FM の前提が崩れたため現行本線ではない。現在の Phase 3 は [12_concept_v2.md](12_concept_v2.md) の decoder adapter / tolerant decoder。以下は旧整合性監査として残す。

## 現状の乖離

| 機能 | 設計 | 実装 |
|---|---|---|
| Phase 3: Progressive RVQ-depth factorized FM heads | 旧CONCEPT「新規性の核心」 | 凍結。残差鎖保持 re-quantization / tolerant decoder へ移行 |
...
- **2026-06-21 status**: 凍結。Phase 1/2 実験で coarse/mid/fine の独立意味分離は否定され、naive token swap は random mix 同等だった。現行タスクは `plan/12_concept_v2.md` の residual-chain-preserving re-quantization と decoder tolerance。
